...
<div id="event-create-container" class="col-md-6 offset-md-3">
    <h1>Criar Produto</h1>  
    <form action="{{ route('produto.store') }}" method="post"> <!-- AQUI ESTÁ O CAMINHO CORRETO -->
        @csrf
        ...
        <div class="form-group">
            ...
            <select class="form-select" id="id_categoria" name="id_categoria" required>
                ...
                @foreach($categoria as $categoria) <!-- Corrigido para usar $categorias -->
                    <option value="{{ $categoria->id_categoria }}">{{ $categoria->nome_categoria }}</option> <!-- Usar o id correto -->
                @endforeach
            </select>
        </div>
